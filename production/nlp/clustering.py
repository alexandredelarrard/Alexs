...
class ClusteringArticles(object):
    ...
    def clean_articles(self):
        ...
        self.articles = self.articles.loc[self.articles["article"].apply(lambda x : len(x)) > 600]
    ...
    def tokenize(self, text):
        ...
        if len(s[0]) < 35:
            text = s[1]
        text = re.sub(r'\r\npar .+\r\n', '', text, flags=re.MULTILINE) # remove autor name
        text = re.sub(r'\r\n\(.+\)', '\r\n', text, flags=re.MULTILINE) # remove photo or end of article credits
        text = re.sub(r'\r\nÀ LIRE AUSSI\r\n.+\r\n', '\r\n', text, flags=re.MULTILINE) ## remove link to other articles
        text = re.sub(r'\r\nLIRE AUSSI\r\n.+\r\n', '\r\n', text, flags=re.MULTILINE) ## remove link to other articles
        text = re.sub(r'www.\S+', '', text, flags=re.MULTILINE) # remove web addresses
        text = text.replace("(Reuters)", "").replace("LIRE NOTRE DOSSIER COMPLET", " ").replace("Article réservé aux abonnés", " ").replace("LIRE NOTRE DOSSIER COMPLET", " ")
    ...
    def clustering_Tf_Itf(self, thresh=0.5, nwords = 50):
        '''
        ...
        '''
        article_words, tfs = self.weight_words(self.articles, nwords)
        ...
        self.articles["cluster"] = index_cluster[1].tolist()
        cluster_unique = self.articles["cluster"].value_counts()[self.articles["cluster"].value_counts() == 1].index
        self.articles["cluster"] = np.where(self.articles["cluster"].isin(cluster_unique), -1 , self.articles["cluster"])
    ...
    def weight_words(self, articles, nwords = 40):
        ...
        tfidf = TfidfVectorizer(stop_words = self.liste_french, preprocessor = self.tokenize, min_df = 3, max_df = 0.35, ngram_range=(1,2))
